...
namespace Build
{
    public class BuildScript : MonoBehaviour
    {
        ...
        private static bool UpdatePackageManifest(string locationPathName)
        {
            ...
            {
                Debug.LogError(string.Format("Unable to find Package.appxmanifest file for build (in path - {0})",
                    buildDir));
                return false;
            }
            ...
            if (identityNode == null)
            {
                Debug.LogError(string.Format(
                    "Package.appxmanifest for build (in path - {0}) is missing an <Identity /> node", buildDir));
                return false;
            }
            ...
            {
                Debug.LogError(string.Format(
                    "Package.appxmanifest for build (in path - {0}) is missing a version attribute in the <Identity /> node.",
                    buildDir));
                return false;
            }
            ...
            var newVersion = new Version(now.Year - 2000, now.Month * 100 + now.Day, now.Hour,
